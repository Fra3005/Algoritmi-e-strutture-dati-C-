...
template<class E, class P>
 ...
 public:
    ...
    ListaNodi listaAdiacenti(Nodo) const;
    ...
    //esercitazione
    int inDegree(Nodo);//numero archi entranti
    int OutDegree(Nodo);//numero archi uscenti
    //visita

    void BFS(Nodo);//stampa in ampiezza partendo da un nodo dato
    //servizio
    double meanOutDegree(Nodo);//media numero archi uscenti per ogni nodo

    /*findPath : Stabilisce se esiste un path da S a D
    e in tal caso lo stampa */
    void findPath(Nodo& u,Nodo& D);
    void stampaDFS(Nodo& u);//algoritmo DFS
...
template<class E,class P>
    typename GrafoMat<E,P>::ListaNodi GrafoMat<E,P>::listaAdiacenti(Nodo n) const{
        ListaNodi lista;
