...
void hash_table<K,E>::insert(mypair<K,E>& the_pair){
   //post: D= DUD' se non esiste coppia (k,v) t.c. K'=K, altrimenti D = D/D' se esiste coppia (k,v)€D t.c K=
...
template<class K, class E>
//pre: se (k',v') €D allora cancella coppia t.c. K =K'
//post: D = D/(k',v')
...
template<class K,class E>
//post: true se non ci sono elementi, false altrimenti
...
template<class K,class E>
List_vector<K> hash_table<K,E>::keys(){
    List_vector<K> l;
    int i =0;
    int j=0;
    for (i=0; i<dsize; i++){
    	while(j<dsize){
    		 l.insert(table[i]->first, l.begin());
    	l.next(j);
    	}
...
/*
template <class K, class E>
//post: vero se esiste (k,v) € D t.c. K' = k, falso altrimenti
bool hash_table <K,E> :: appartiene(const K &k){

	 string* chiavi = this->keys();
