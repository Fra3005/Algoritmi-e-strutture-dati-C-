...
public:
	...
	int liv(Nodo);
	int numeroFigli(Nodo);
	List_vector<T> figliPari(Nodo);
...
Cella<T>* Albero<T>::padre(Nodo n) {
	if (n != radiceAlbero()) {
		return n->getPadre(); // n non è radice
	}
	...
	if (radiceAlbero() == n)
		...
		if (n == primoFiglio(padre(n))) { //se n è il primo figlio
			if (ultimoFratello(n)) //e non ha fratelli
				...
				//altrimenti ha un fratello che diventa primo figlio
...
int Albero<T>::nodiKfigli(Nodo n, int k) {
	...
	if (n == NULL)
		...
		if (k == k1)
			return (1 + nodiKfigli(succFratello(n), k)
					+ nodiKfigli(primoFiglio(n), k));
		else
			return (nodiKfigli(succFratello(n), k)
					+ nodiKfigli(primoFiglio(n), k));
	}
}
...
bool Albero<T>::pariDispari(Nodo n) {
	...
	if (leggiNodo(n) % 2 == 0) {
		...
		temp = primoFiglio(n);
		while (temp != NULL) {
			flag = flag && (leggiNodo(temp) % 2) && pariDispari(temp);
			temp = succFratello(temp);
		}
		return flag;
	}
...
template<class T>
int Albero<T>::numeroFigli(Nodo n) {
	int k = 0;
	if (foglia(n)) {
		return 0;
	} else {
...
template<class T>
List_vector<T> Albero<T>::figliPari(Nodo n) {
	List_vector<T> lista;
	if (numeroFigli(n) < 3) {
		lista.insert(n->getElemento(), lista.begin());
	}
	Nodo temp = primoFiglio(n);
	while (temp != NULL) {
		if (numeroFigli(temp) < 3) {
			lista.insert(temp->getElemento(), lista.begin());
		}
